...
protected:
    ...
    PFLocalization* particleFilter;
     ...
     // 每场踢球重新调整后初始化的私有成员变量 ， 用来判断开球时间
	 double timeStart;
	 double INIT_WAIT_TIME; // 模仿optimization.cc中写的，是为了踢球之前站稳而设定的，实际上这边不需要。
	 bool KICKorNot;		// 为了与多支代码测试而设置的参数，开球
	 bool hasWrite;		// 写入文件
	 double endTime;
	 bool ShouldShoot(bool &shouldFastKick);
