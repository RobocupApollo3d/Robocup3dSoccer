...
#define AUTO_TEST
#define AUTO_FILE  "./test/SCORE.log"
...
//    cout << "****************************************\n";
    ...
    */
    ...
    act();
        ...
        /**
         * @author Mrli
         * 只有11号具有与平台交互的能力，自动测试
         */
          if(worldModel->getUNum() == WO_TEAMMATE11 ) {
        /**
        * 发送开球命令
        */
          if( ! KICKorNot  && worldModel->getWorldObject(WO_OPPONENT11)->validPosition){
        // 判断对方11号是否在场上，等对方BEAM完成后再开球
                KICKorNot = true;
        ...
        * 将测试结果写入"SCORE.log"
        */
        if( KICKorNot && worldModel->getPlayMode() == PM_BEFORE_KICK_OFF && !hasWrite && worldModel->getGameTime() > 1.0f){
            stringstream ss;
            fstream fp(AUTO_FILE,ios::app);
            if (worldModel->getSide() == SIDE_LEFT) {
              ss << worldModel->getScoreLeft();
              fp << agentTeamName << " VS " << worldModel->getOpponentTeamName() << '\t' << ss.str();
              ss.str("");
              ss << worldModel->getScoreRight();
             fp << " : " << ss.str() << endl;

            }else {	// 己方为左边比分
              ss << worldModel->getScoreLeft();
              fp  << worldModel->getOpponentTeamName() <<" VS " << agentTeamName << '\t' << ss.str();
              ss.str("");
              ss << worldModel->getScoreRight();
              fp << " : " << ss.str() << endl;
            }
