...
then
    # 双方对抗上人 , 己方上优化agent
    $DIR_SCRIPT/../agentspark --unum 1 --type $TYPE --paramsfile $DIR_SCRIPT/../paramfiles/defaultParams.txt --paramsfile $DIR_SCRIPT/../paramfiles/defaultParams_t$TYPE.txt --paramsfile $PARAMS_FILE --experimentout $OUTPUT_FILE --optimize kickModelAgent --port $SPARK_AGENTPORT --mport $SPARK_SERVERPORT &

    sleep 1
    # 对方上人
    $DIR_SCRIPT/2018_Bin/$opp/start.sh --port $SPARK_AGENTPORT &
fi
...
then
    ...
    # 优化了踢球
    $DIR_SCRIPT/../agentspark --unum 1 --type $TYPE --paramsfile $DIR_SCRIPT/../paramfiles/defaultParams.txt --paramsfile $DIR_SCRIPT/../paramfiles/defaultParams_t$TYPE.txt --paramsfile $PARAMS_FILE --experimentout $OUTPUT_FILE --optimize fixedKickAgent --port $SPARK_AGENTPORT --mport $SPARK_SERVERPORT &
fi
