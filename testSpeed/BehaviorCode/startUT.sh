...
host="localhost"
port=3100
paramsfile=paramfilesUT/defaultParams.txt
mhost="localhost"
mport=12345
type=0
...
case  $type in
  0)
      "$BINARY_DIR/$AGENT_BINARY" $opt --unum $i --type $type --paramsfile paramfilesUT/defaultParams_t0.txt &#> /dev/null &
      #"$BINARY_DIR/$AGENT_BINARY" $opt --unum $i --type 0 --paramsfile paramfiles/defaultParams_t0.txt > stdout$i 2> stderr$i &
      ;;
  1)
      "$BINARY_DIR/$AGENT_BINARY" $opt --unum $i --type 1 --paramsfile paramfilesUT/defaultParams_t1.txt &#>  /dev/null &
      #"$BINARY_DIR/$AGENT_BINARY" $opt --unum $i --type 1 --paramsfile paramfiles/defaultParams_t1.txt > stdout$i 2> stderr$i &
      ;;
  2)
      "$BINARY_DIR/$AGENT_BINARY" $opt --unum $i --type 2 --paramsfile paramfilesUT/defaultParams_t2.txt &#> /dev/null &
      #"$BINARY_DIR/$AGENT_BINARY" $opt --unum $i --type 2 --paramsfile paramfiles/defaultParams_t2.txt > stdout$i 2> stderr$i &
      ;;
  3)
      "$BINARY_DIR/$AGENT_BINARY" $opt --unum $i --type 3 --paramsfile paramfilesUT/defaultParams_t3.txt &#> /dev/null &
      #"$BINARY_DIR/$AGENT_BINARY" $opt --unum $i --type 3 --paramsfile paramfiles/defaultParams_t3.txt > stdout$i 2> stderr$i &
      ;;
  4)
      "$BINARY_DIR/$AGENT_BINARY" $opt --unum $i --type 4 --paramsfile paramfilesUT/defaultParams_t4.txt &#> /dev/null &
