...
opt="${opt} --host=${host} --port ${SPARK_AGENTPORT} --team ${team} ${paramsfile_args} --mhost=${mhost} --mport ${mport}"
...
for ((i=1;i<=$NUM_PLAYERS;i++)); do
    case $i in
      # 3|5|9)
	2|5|9)
	    echo "Running Apollo_agent No. $i -- Type 0"
	...
	3|7|8|10)
  # 7|8|10)
	    echo "Running Apollo_agent No. $i -- Type 1"
	    ...
	    ;;
	*)
	    echo "Running Apollo_agent No. $i -- Type 4"
