...
#/home/username/RoboViz-dev/bin/linux-amd64/roboviz.sh --serverPort=$SPARK_SERVERPORT &
...
# do
  ...
  for((i=1;i<=$TIME;i++));  
    ...
    # 己方上人
    ./mport.sh --mport $SPARK_SERVERPORT --port $SPARK_AGENTPORT &
    sleep 15
    ...
    if [ -f $LOG_FILE_PATH ];then
        echo "YES"
        mv $LOG_FILE_PATH $LOG_FOLDER_PATH/$i-1.log
    fi
    ...
    if [ -f $LOG_FILE_PATH ];then
        echo "YES"
        mv $LOG_FILE_PATH $LOG_FOLDER_PATH/$i-2.log
